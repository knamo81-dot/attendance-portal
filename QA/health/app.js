(() => {
  ...
  // 월 셀 클릭은 렌더링 후 개별 바인딩 대신 tbody 이벤트 위임으로 처리한다.
  // 셀 내부의 예약/완료 카드, 날짜, 병원명을 클릭해도 항상 상세창이 열린다.
  $('#statusBody')?.addEventListener('click',e=>{
    const cell=e.target.closest?.('.month-cell');
    if(!cell||!$('#statusBody').contains(cell))return;
    e.preventDefault();
    e.stopPropagation();
    openDrawer(cell.dataset.emp,+cell.dataset.month);
  });
