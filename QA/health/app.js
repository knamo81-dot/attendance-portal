(() => {
  ...
  function updateTeams(){const div=$('#statusDivision').value,cur=$('#statusTeam').value,vals=[...new Set(state.status.employees.filter(e=>!div||e.division_code===div).map(e=>e.team_code).filter(Boolean))].sort();$('#statusTeam').innerHTML='<option value="">전체</option>'+vals.map(v=>`<option>${esc(v)}</option>`).join('');if(vals.includes(cur))$('#statusTeam').value=cur}
  function fillStatusFilters(){const y=$('#statusYear'),now=new Date().getFullYear();y.innerHTML='';for(let n=now+1;n>=now-5;n--)y.insertAdjacentHTML('beforeend',`<option value="${n}">${n}년</option>`);y.value=state.status.year;const ds=[...new Set(state.status.employees.map(e=>e.division_code).filter(Boolean))].sort();$('#statusDivision').innerHTML='<option value="">전체</option>'+ds.map(v=>`<option>${esc(v)}</option>`).join('');updateTeams()}
  function visibleEmps(){const d=$('#statusDivision').value,t=$('#statusTeam').value,q=$('#statusSearch').value.trim().toLowerCase();return state.status.employees.filter(e=>isTrue(e.special_health_exam_target)&&e.status!=='퇴사'&&!activeAbsence(e.employee_no)).filter(e=>(!d||e.division_code===d)&&(!t||e.team_code===t)&&(!q||`${e.name||''} ${e.employee_no||''}`.toLowerCase().includes(q))).sort((a,b)=>Number(a.sort_order||9999)-Number(b.sort_order||9999)||String(a.name||'').localeCompare(String(b.name||''),'ko'))}
  function renderStatus(){if(!state.status.loaded)return;const year=+$('#statusYear').value;state.status.year=year;const rows=visibleEmps(),now=today();let doneP=0,dueN=0,overN=0;$('#statusBody').innerHTML=rows.map(emp=>{const hz=empHazards(emp),ex=state.status.exams.filter(e=>String(e.employee_no)===String(emp.employee_no)&&yearOf(e.exam_date)===year);if(ex.length)doneP++;const ev=Array.from({length:12},()=>[]);ex.forEach(e=>ev[monthOf(e.exam_date)-1]?.push({type:'done',exam:e}));hz.forEach(h=>{if(yearOf(h.due)===year){const type=h.due<now?'overdue':'due';ev[monthOf(h.due)-1]?.push({type,h});type==='overdue'?overN++:dueN++}});const next=hz.map(h=>h.due).sort()[0]||null;const months=ev.map((a,i)=>{if(!a.length)return`<td class="month-cell" data-emp="${esc(emp.employee_no)}" data-month="${i+1}"></td>`;const dn=a.filter(x=>x.type==='done').length,on=a.filter(x=>x.type==='overdue').length,un=a.filter(x=>x.type==='due').length,type=on?'overdue':dn?'done':'due',label=[dn&&`완료 ${dn}`,on&&`초과 ${on}`,un&&`예정 ${un}`].filter(Boolean).join(' ');return`<td class="month-cell" data-emp="${esc(emp.employee_no)}" data-month="${i+1}"><span class="month-chip ${type}">${label}</span></td>`}).join('');return`<tr><td class="employee-cell"><strong>${esc(emp.name||'-')}</strong><small>${esc(emp.employee_no)} · ${esc(emp.division_code||'-')} / ${esc(emp.team_code||'-')}</small></td><td><span class="next-date ${next&&next<now?'overdue':''}">${esc(next||'-')}</span></td>${months}</tr>`}).join('')||'<tr><td colspan="14" class="empty">조건에 해당하는 특수검진 대상자가 없습니다.</td></tr>';$('#statusTargetCount').textContent=rows.length+'명';$('#statusDoneCount').textContent=doneP+'명';$('#statusDueCount').textContent=dueN+'건';$('#statusOverdueCount').textContent=overN+'건';$$('.month-cell').forEach(x=>x.addEventListener('click',()=>openDrawer(x.dataset.emp,+x.dataset.month)))}
  function openDrawer(no,m){const emp=state.status.employees.find(e=>String(e.employee_no)===String(no)),year=state.status.year,hz=empHazards(emp),ex=state.status.exams.filter(e=>String(e.employee_no)===String(no)&&yearOf(e.exam_date)===year&&monthOf(e.exam_date)===m),due=hz.filter(h=>yearOf(h.due)===year&&monthOf(h.due)===m);$('#drawerName').textContent=`${emp.name||'-'} · ${year}년 ${m}월`;$('#drawerMeta').textContent=`${emp.employee_no} · ${emp.division_code||'-'} / ${emp.team_code||'-'} · 입사일 ${emp.join_date||'-'}`;const exHtml=ex.length?ex.map(e=>{const hs=state.status.examHazards.filter(h=>String(h.exam_id)===String(e.id)).map(h=>state.status.hazards.find(x=>String(x.chemical_id)===String(h.chemical_id))).filter(Boolean);return`<div class="drawer-hazard"><b>${esc(dateOnly(e.exam_date))} 검진완료</b><small>${esc(e.exam_institution||'검진기관 미입력')} · ${esc(hs.map(x=>x.cas_no+' '+(x.name_ko||x.name_en||'')).join(', ')||'유해인자 미등록')}</small></div>`}).join(''):'<div class="drawer-hazard">해당 월 검진이력이 없습니다.</div>';const dueHtml=due.length?due.map(h=>`<div class="drawer-hazard"><b>${esc(h.cas_no)} · ${esc(h.name_ko||h.name_en||'-')}</b><small>${h.kind==='first'?'최초검진':'정기검진'} · 기한 ${esc(h.due)} · 주기 ${esc(h.exam_cycle_months||'-')}개월${h.last?' · 최근검진 '+esc(h.last):''}</small></div>`).join(''):'<div class="drawer-hazard">해당 월 예정 유해인자가 없습니다.</div>';$('#drawerBody').innerHTML=`<section class="drawer-section"><h3>검진이력</h3>${exHtml}</section><section class="drawer-section"><h3>검진예정 유해인자</h3>${dueHtml}</section><section class="drawer-section"><h3>현재 적용 유해인자</h3>${hz.length?hz.map(h=>`<div class="drawer-row"><span>${esc(h.cas_no)}</span><b>${esc(h.name_ko||h.name_en||'-')}</b></div>`).join(''):'<div class="drawer-hazard">최근 12개월 사용기록 기준 적용 유해인자가 없습니다.</div>'}</section>`;$('#healthDrawer').classList.remove('hidden');$('#healthDrawerBackdrop').classList.remove('hidden')}
  function closeDrawer(){$('#healthDrawer').classList.add('hidden');$('#healthDrawerBackdrop').classList.add('hidden')}
  async function loadStatus(){const sb=client();if(!sb)return;try{const cid=companyId();let eq=sb.from('employees').select('*');if(cid)eq=eq.eq('company_id',cid);const [er,nr,xr,hr,sr,cr,pr,ur]=await Promise.all([eq,sb.from('employee_special_notes').select('*'),sb.from('qa_special_health_exams').select('*'),sb.from('qa_special_health_exam_hazards').select('*'),sb.from('qa_special_health_exam_standards').select('*'),sb.from('qa_chemical_master').select('id,cas_no,chem_name_ko,chem_name_en'),sb.from('product_cas').select('product_id,cas_no'),sb.from('qa_reagent_usage_records').select('product_id,usage_date')]);for(const r of[er,nr,xr,hr,sr,cr,pr,ur])if(r.error)throw r.error;state.status.employees=er.data||[];state.status.notes=(nr.data||[]).filter(n=>!cid||!n.company_id||String(n.company_id)===cid);state.status.exams=(xr.data||[]).filter(e=>!cid||String(e.company_id)===cid);state.status.examHazards=hr.data||[];const cm=new Map((cr.data||[]).map(c=>[c.id,c])),cp=new Map(),up=new Map();(pr.data||[]).forEach(x=>{const a=cp.get(x.cas_no)||[];a.push(x.product_id);cp.set(x.cas_no,a)});(ur.data||[]).forEach(u=>{if(!u.usage_date)return;const a=up.get(u.product_id)||[];a.push(dateOnly(u.usage_date));up.set(u.product_id,a)});const cut=new Date();cut.setFullYear(cut.getFullYear()-1);const cs=cut.toISOString().slice(0,10);state.status.hazards=(sr.data||[]).filter(s=>s.is_target===true&&s.status!=='inactive'&&s.verification_status!=='kosha_not_found').map(s=>{const c=cm.get(s.chemical_id)||{},dates=(cp.get(c.cas_no)||[]).flatMap(id=>up.get(id)||[]).sort();return{...s,cas_no:c.cas_no||'',name_ko:c.chem_name_ko||'',name_en:c.chem_name_en||'',first_usage:dates[0]||null,latest_usage:dates.at(-1)||null,active_usage:dates.some(d=>d>=cs)}});state.status.loaded=true;fillStatusFilters();renderStatus()}catch(e){console.error(e);$('#statusBody').innerHTML=`<tr><td colspan="14" class="empty">현황 조회 실패: ${esc(e.message||e)}</td></tr>`;toast('특수건강진단 현황을 불러오지 못했습니다.',true)}}
