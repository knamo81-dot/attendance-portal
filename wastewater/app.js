(function injectRelatedDocDetailStyle(){
  ...
  style.id='related-doc-detail-style';
  style.textContent=`
    .related-doc-title-link{border:0;background:transparent;padding:0;color:#071b49;font-weight:900;cursor:pointer;text-align:left}
    .related-doc-title-link:hover{text-decoration:underline}
    .related-doc-detail{border:1px solid var(--line);border-radius:18px;background:#fff;padding:22px}
    .related-doc-detail-category{font-size:12px;font-weight:900;color:#4f6b93;margin-bottom:8px}
    .related-doc-detail-title{font-size:24px;font-weight:900;color:var(--navy);letter-spacing:-.03em;margin-bottom:8px}
    .related-doc-detail-body{margin-top:18px;border-top:1px solid var(--line);padding-top:18px;min-height:180px}
    .related-doc-detail-body img{max-width:100%;height:auto;border-radius:14px;border:1px solid var(--line);padding:6px;background:#fff}
  `;
...
let user=null,currentRole='user',currentRoles=[],currentTab='daily',ledgerMonthFilter='',pickupYearFilter='',pickupTypeFilter='전체',yearlyReportFilter='',refTab='facilities',refSearch='',relatedDocCategoryFilter='삼천당계약 사업자등록증',relatedDocWriteMode=false,relatedDocViewId=null,relatedDocEditId=null;
...
function roleLabel(role){
  ...
  if(role==='operator' || role==='wastewater_operator') return '폐수 담당자';
  if(role==='supervisor' || role==='wastewater_approver') return '최종 결재자';
  if(role==='admin' || role==='wastewater_admin') return '폐수 관리자';
  return role;
}
function roleBadgeClass(role){
  if(role==='admin' || role==='wastewater_admin') return 'admin';
  if(role==='operator' || role==='wastewater_operator') return 'operator';
  if(role==='supervisor' || role==='wastewater_approver') return 'supervisor';
  return 'user';
}
function roleBadge(role){return `<span class="role-badge ${roleBadgeClass(role)}">${escapeHtml(roleLabel(role))}</span>`;}
function currentRoleBadges(){
  if(!currentRoles.length) return roleBadge('user');
  const order=['wastewater_admin','wastewater_operator','wastewater_approver'];
  const sorted=[...currentRoles].sort((a,b)=>order.indexOf(a)-order.indexOf(b));
  return sorted.map(role=>roleBadge(role)).join(' ');
}
function syncCurrentRole(){
  if(currentRoles.includes('wastewater_admin')) currentRole='admin';
  else if(currentRoles.includes('wastewater_operator')) currentRole='operator';
  else if(currentRoles.includes('wastewater_approver')) currentRole='supervisor';
...
function canDownload(){return hasRole('wastewater_admin')||hasRole('wastewater_operator')||hasRole('wastewater_approver');}
function canWriterApprove(){return hasRole('wastewater_admin')||hasRole('wastewater_operator');}
function canReviewerApprove(){return hasRole('wastewater_admin')||hasRole('wastewater_approver');}
function monthKeyFromDate(v){return String(v||'').slice(0,7);}
function yearKeyFromDate(v){return String(v||'').slice(0,4);}
function getDefaultLedgerMonth(){const d=dailyRows.length?dailyRows[0].date:new Date().toISOString().slice(0,10);return monthKeyFromDate(d);}
function getPickupYears(){const years=[...new Set(pickupRows.map(r=>yearKeyFromDate(r.pickup_date)).filter(Boolean))].sort((a,b)=>Number(b)-Number(a)); const cy=String(new Date().getFullYear()); if(!years.includes(cy))years.unshift(cy); return years;}
function getDefaultPickupYear(){return getPickupYears()[0]||String(new Date().getFullYear());}
function getAvailableYears(){const years=[...new Set([...dailyRows.map(r=>yearKeyFromDate(r.date)).filter(Boolean),...pickupRows.map(r=>yearKeyFromDate(r.pickup_date)).filter(Boolean)])].sort((a,b)=>Number(b)-Number(a)); const cy=String(new Date().getFullYear()); if(!years.includes(cy))years.unshift(cy); return years;}
function getDefaultYearlyReportYear(){return getAvailableYears()[0]||String(new Date().getFullYear());}
function formatMonthLabel(m){if(!m||m.length<7)return m||'-'; return `${m.slice(0,4)}년 ${Number(m.slice(5,7))}월`;}
function safeNumber(v){const n=Number(v); return Number.isFinite(n)?n:0;}
function summarizeWasteCounts(rows){return rows.reduce((a,r)=>{a.alkali+=safeNumber(r.waste_alkali_count);a.acid+=safeNumber(r.waste_acid_count);a.organicSolid+=safeNumber(r.waste_organic_solid_count);a.organicBottle+=safeNumber(r.waste_organic_bottle_count);return a;},{alkali:0,acid:0,organicSolid:0,organicBottle:0});}
function getApprovalRow(monthKey){return approvalRows.find(r=>r.month_key===monthKey)||null;}
...
function getDefaultReferenceMap(){
  return Object.fromEntries(Object.entries(DEFAULT_REFERENCE_DOCS).map(([key,val])=>[
    key,
    {
      doc_key:key,
      title:val.title,
      body:(val.lines||[]).join('\n'),
      source:val.source||'기본 내장 문서',
      updated_at:null,
      updated_by:null
    }
  ]));
...
function applyUrlState(){
  try{
    ...
    if(month) ledgerMonthFilter = month;
  }catch(e){}
}
function syncUrlState(){
  try{
    ...
    if(ledgerMonthFilter) params.set('month', ledgerMonthFilter);
    const newUrl = `${location.pathname}?${params.toString()}`;
    history.replaceState({},'',newUrl);
  }catch(e){}
}
function getReviewerCandidateEmail(){
  const candidates = userRows.filter(u => (u.role_name==='wastewater_approver' || u.role_name==='wastewater_admin') && u.email !== user?.email);
  return candidates[0]?.email || '';
}
function buildLedgerLink(monthKey){
  const base = `${location.origin}${location.pathname}`;
  return `${base}?tab=ledger&month=${encodeURIComponent(monthKey)}`;
}
function sendReviewerRequestMail(monthKey){
  return sendApprovalMail(monthKey);
}

async function getMyRoles(email){
  return api.getMyRoles(email);
}
...
async function sendApprovalMail(monthKey){
  return renderApp('메일 기능은 사용하지 않습니다.','muted');
}


function readPortalUserFromStorage(){
  const keys=['portal_auth_user','portalUser','attendance_portal_user'];
  for(const key of keys){
    try{
      ...
      if(email) return {email, name: parsed?.name||parsed?.user?.name||''};
    }catch(e){}
  }
  return null;
}
...
function renderPortalAccessRequired(message='',cls='muted'){
  document.getElementById('app').innerHTML=`<div class="portal-access-wrap"><div class="card portal-access-card"><h2>포탈 로그인 연동 대기</h2><p>이 화면은 폐수 앱 자체 로그인을 사용하지 않습니다.<br>포탈에서 로그인한 뒤 접속하거나, 포탈이 사용자 정보를 전달해야 정상적으로 열립니다.</p><div class="portal-access-help">${escapeHtml(message||'현재는 포탈 로그인 사용자 정보가 전달되지 않았습니다. 포탈에서 진입해 주세요.')}</div></div></div>`;
}

function requestPortalUserFromParent(){
  try{
    if(window.parent && window.parent!==window){
      window.parent.postMessage({type:'portal-auth-request', app:'wastewater'}, '*');
    }
  }catch(e){}
}
...
function renderLogin(message='',cls='muted'){renderPortalAccessRequired(message,cls);}
async function login(){renderPortalAccessRequired('이 화면은 포탈 로그인 연동 전용입니다. 포탈에서 접속해 주세요.','err');}
async function signup(){renderPortalAccessRequired('이 화면은 회원가입을 사용하지 않습니다. 포탈에서 접속해 주세요.','err');}
async function logout(){
  try{
    if(window.parent && window.parent!==window){
      window.parent.postMessage({type:'portal-logout-request', app:'wastewater'}, '*');
    }
  }catch(e){}
  renderPortalAccessRequired('포탈에서 다시 접속해 주세요.','muted');
}
async function getUserRowByEmail(email){const roles=await getMyRoles(email); return roles.length?{email,roles}:null;}
async function ensureUserRow(email){return {email};}
async function loadMyRole(){await loadMyRoles();}
...
function getYearlyReportData(year){
  ...
  const allMonthKeys=Array.from({length:12},(_,i)=>`${year}-${String(i+1).padStart(2,'0')}`);
  const monthlyMap={};

  allMonthKeys.forEach(mk=>{
    const monthNum=Number(mk.slice(5,7));
    monthlyMap[mk]={monthKey:mk,label:`${monthNum}월`,waterUse:0,entrusted:0,overCount:0,wasteAlkali:0,wasteAcid:0,wasteOrganicSolid:0,wasteOrganicBottle:0,included:closedMonths.includes(mk)};
  });

  const yearLedgerRows=stats.enrichedDaily.filter(r=>closedMonths.includes(monthKeyFromDate(r.date)));
  const yearPickupRows=pickupRows.filter(r=>closedMonths.includes(monthKeyFromDate(r.pickup_date)));

  yearLedgerRows.forEach(r=>{
    const mk=monthKeyFromDate(r.date);
    if(!monthlyMap[mk]) return;
    monthlyMap[mk].waterUse+=safeNumber(r.water_used);
    if(safeNumber(r.total_cm)>CM_LIMIT) monthlyMap[mk].overCount+=1;
  });

  yearPickupRows.forEach(r=>{
    const mk=monthKeyFromDate(r.pickup_date);
    if(!monthlyMap[mk]) return;
    const type=r.pickup_type||'폐수';
    if(type==='폐수') monthlyMap[mk].entrusted+=safeNumber(r.entrusted_amount);
    else{
      monthlyMap[mk].wasteAlkali+=safeNumber(r.waste_alkali_count);
      monthlyMap[mk].wasteAcid+=safeNumber(r.waste_acid_count);
      monthlyMap[mk].wasteOrganicSolid+=safeNumber(r.waste_organic_solid_count);
      monthlyMap[mk].wasteOrganicBottle+=safeNumber(r.waste_organic_bottle_count);
    }
  });

  const allMonthlyRows=Object.values(monthlyMap).map(r=>({...r,waterUse:round2(r.waterUse),entrusted:round2(r.entrusted)}));
  const monthlyRows=allMonthlyRows.filter(r=>r.included);

  const totalWaterUse=round2(monthlyRows.reduce((s,r)=>s+safeNumber(r.waterUse),0));
  const totalEntrusted=round2(monthlyRows.reduce((s,r)=>s+safeNumber(r.entrusted),0));
  const totalOverCount=monthlyRows.reduce((s,r)=>s+safeNumber(r.overCount),0);
  const wasteTotals=monthlyRows.reduce((a,r)=>{a.alkali+=safeNumber(r.wasteAlkali);a.acid+=safeNumber(r.wasteAcid);a.organicSolid+=safeNumber(r.wasteOrganicSolid);a.organicBottle+=safeNumber(r.wasteOrganicBottle);return a;},{alkali:0,acid:0,organicSolid:0,organicBottle:0});
  const safeTop=(rows,key)=>rows.length?[...rows].sort((a,b)=>safeNumber(b[key])-safeNumber(a[key]))[0]:null;
  ...
  return {
    ...
    totalOverCount,
    wasteTotals,
    topOverMonth:safeTop(monthlyRows,'overCount'),
    topWaterMonth:safeTop(monthlyRows,'waterUse'),
    topEntrustedMonth:safeTop(monthlyRows,'entrusted')
  };
}

function getYearApprovalSummary(year){
  const months = Array.from({length:12}, (_,i)=>`${year}-${String(i+1).padStart(2,'0')}`);
  const completedMonths = months.filter(monthKey=>{
    const row = getApprovalRow(monthKey);
    return !!(row?.writer_signed_at && row?.reviewer_signed_at);
  });
  const pendingMonths = months.filter(monthKey=>!completedMonths.includes(monthKey));
  return {
    ...
    completedMonths,
    pendingMonths,
    isComplete: pendingMonths.length===0
  };
}

function renderPickupRows(rows){
  if(!rows.length) return `<tr><td colspan="7">선택한 조건의 수거 내역이 없습니다.</td></tr>`;
  return rows.map(row=>`<tr><td>${escapeHtml(row.pickup_date)}</td><td>${getPickupTypeBadge(row.pickup_type||'폐수')}</td><td>${escapeHtml(getPickupDetailText(row))}</td><td>${escapeHtml(row.certificate_no||'')}</td><td>${escapeHtml(row.contractor||'')}</td><td>${escapeHtml(row.note||'')}</td><td>${canDelete()?`<button class="small-btn danger" onclick="deletePickupRow(${row.id})">삭제</button>`:'-'}</td></tr>`).join('');
}
...
function getReferenceDoc(){
  ...
  const defaultDoc = DEFAULT_REFERENCE_DOCS[refTab] || { title:'', source:'', lines:[] };

  return {
    doc_key: refTab,
    title: defaultDoc.title || '',
    body: (defaultDoc.lines || []).join('\n'),
    source: defaultDoc.source || '기본 내장 문서',
    updated_at: null,
    updated_by: null
  };
}
...
function normalizeReferenceBodyForEditor(body){
  const raw=String(body||'').trim();
  if(!raw) return '';
  if(/<([a-z][\w-]*)(\s|>)/i.test(raw)) return raw;
  return raw.split(/\r?\n/).map(line=>{
    const t=line.trim();
    if(!t) return '<p><br></p>';
...
function getFilteredReferenceLinesFromBody(bodyText){
  const plain = stripHtml(normalizeReferenceBodyForEditor(bodyText));
  ...
  const cleaned = lines.filter(line=>{
    ...
    if(/^#VALUE!(\s+#VALUE!)*$/i.test(normalized)) return false;
    return true;
  });
  if(!refSearch.trim()) return cleaned;
  const q=refSearch.trim().toLowerCase();
  return cleaned.filter(line=>String(line).toLowerCase().includes(q));
}

function renderDocBodyFromText(bodyText){
  const normalized = normalizeReferenceBodyForEditor(bodyText);
  if(!normalized) return `<div class="doc-empty">내용이 없습니다.</div>`;
  if(/<([a-z][\w-]*)(\s|>)/i.test(String(normalized||''))){
    return `<div class="doc-html">${normalized}</div>`;
  }

  const lines = getFilteredReferenceLinesFromBody(bodyText);
  if(!lines.length) return `<div class="doc-empty">검색 결과가 없습니다.</div>`;
  ...
  return lines.map(rawLine=>{
    ...
    if (/^Q\d+[\.\)]?/i.test(trimmed) || /^Q\./i.test(trimmed) || trimmed.startsWith('Q ')){
      mode = 'Q';
      return `<div class="doc-question">${escaped}</div>`;
    }
    if (/^A[\.\)]?/i.test(trimmed) || trimmed.startsWith('A ')){
      mode = 'A';
      return `<div class="doc-answer">${escaped}</div>`;
    }
    if (
      ...
      mode = null;
      return `<div class="doc-section">${escaped}</div>`;
    }
    if (mode === 'A'){
      return `<div class="doc-answer">${escaped}</div>`;
    }
    return `<div class="doc-text">${escaped}</div>`;
  }).join('');
}

function getRefEditorHtml(){
  const el = document.getElementById('ref-body-input');
  return el ? el.innerHTML : '';
}

function execEditorCommand(command, value=null){
  const editor = document.getElementById('ref-body-input');
  if(!editor) return;
  editor.focus();
...
function applyEditorLineHeight(value){
  const editor = document.getElementById('ref-body-input');
  ...
  while(target && target!==editor && !/^(P|DIV|LI|H1|H2|H3|BLOCKQUOTE|SPAN)$/i.test(target.tagName||'')){
...
function editorUndo(){ execEditorCommand('undo'); }
function editorRedo(){ execEditorCommand('redo'); }
...
function triggerEditorImageUpload(){
  const input = document.getElementById('ref-image-upload');
...
function handleEditorImage(file){
  ...
  reader.onload = function(){
    ...
    execEditorCommand('insertImage', src);
  };
...
function formatText(type){
  ...
  if(!editor) return;
  editor.focus();
  if(type === 'bold') return execEditorCommand('bold');
  if(type === 'left') return execEditorCommand('justifyLeft');
  if(type === 'center') return execEditorCommand('justifyCenter');
  if(type === 'right') return execEditorCommand('justifyRight');
}

async function saveReferenceDoc(){
    if(!canAdmin()) return renderApp('관리자만 운영 관련자료를 수정할 수 있습니다.','err');
  const title = document.getElementById('ref-title-input')?.value?.trim() || '';
  const body = getRefEditorHtml();
  if(!title) return renderApp('문서 제목을 입력하세요.','err');
  ...
  const existing = referenceDocs.find(d=>d.doc_key===refTab);
  let result;
  if(existing?.id){
    result = await api.saveReferenceDoc(existing.id,payload);
  }else{
    result = await api.saveReferenceDoc(null,payload);
  }
  if(result.error) return renderApp('운영 관련자료 저장 실패: ' + result.error.message,'err');
  await logActivity('save','reference_doc',refTab,{title});
  await loadAll('운영 관련자료 저장 완료','ok');
}

async function deleteReferenceDoc(){
  if(!canAdmin()) return renderApp('관리자만 운영 관련자료를 삭제할 수 있습니다.','err');
  const existing = referenceDocs.find(d=>d.doc_key===refTab);
  if(!existing?.id){
    return renderApp('삭제할 사용자 작성 문서가 없습니다. 현재 문서는 기본 빈 문서 상태입니다.','err');
  }
  const ok = confirm('현재 운영 관련자료 문서를 삭제하고 빈 기본 상태로 되돌릴까요?');
  if(!ok) return;
  const result = await api.deleteReferenceDocById(existing.id);
  if(result.error) return renderApp('운영 관련자료 삭제 실패: ' + result.error.message,'err');
  await logActivity('delete','reference_doc',refTab,{title:existing.title||'', deleted_id:existing.id});
  await loadAll('운영 관련자료 삭제 완료. 빈 기본 상태로 복원되었습니다.','ok');
}
  
function getPendingReviewerApprovalMonths(){
  if(!canReviewerApprove()) return [];
  return approvalRows
    .filter(row=>row?.writer_signed_at && !row?.reviewer_signed_at)
    .map(row=>row.month_key)
    .filter(Boolean)
    .sort((a,b)=>String(b).localeCompare(String(a)));
}
function buildApprovalAlertModal(){
  if(approvalAlertDismissed) return '';
  const pendingMonths=getPendingReviewerApprovalMonths();
  if(!pendingMonths.length) return '';
  const firstMonth=pendingMonths[0];
  return `<div class="approval-alert-overlay" id="approval-alert-overlay">
    <div class="approval-alert-modal" role="dialog" aria-modal="true" aria-labelledby="approval-alert-title">
      <div class="approval-alert-head">
        <div class="approval-alert-eyebrow">결재 대기 알림</div>
        <h2 class="approval-alert-title" id="approval-alert-title">최종 결재가 필요한 월간 보고서가 있습니다</h2>
      </div>
      <div class="approval-alert-body">
        <p class="approval-alert-desc">작성자 결재가 완료된 보고서가 있습니다. 아래 대상 월을 확인하고 운영일지에서 최종 결재를 진행해주세요.</p>
        <ul class="approval-alert-list">
          ${pendingMonths.slice(0,6).map(monthKey=>`<li class="approval-alert-item"><span class="inline-badge partial">최종 결재 대기</span><span class="approval-alert-month">${formatMonthLabel(monthKey)}</span></li>`).join('')}
        </ul>
      </div>
      <div class="approval-alert-foot">
        <button class="btn" onclick="dismissApprovalAlert()">나중에 보기</button>
        <button class="btn primary" onclick="goToPendingApproval('${firstMonth}')">운영일지로 이동</button>
      </div>
    </div>
  </div>`;
}
function dismissApprovalAlert(){
  approvalAlertDismissed=true;
  const el=document.getElementById('approval-alert-overlay');
  if(el) el.remove();
}
function goToPendingApproval(monthKey){
  approvalAlertDismissed=true;
  ledgerMonthFilter=monthKey||ledgerMonthFilter;
  currentTab='ledger';
  renderApp();
  syncUrlState();
}
...
function formatAdminDate(value){
  ...
  return Number.isNaN(d.getTime()) ? String(value).slice(0,10) : d.toLocaleDateString('ko-KR');
}
function getCurrentActorName(){return user?.name || user?.email || '관리자';}
function getManagerEmployeeNo(row={}){return row.employee_no || row.employeeNo || '';}
function getManagerEmail(row={}){return row.email || row.user_email || '';}
function getManagerDepartment(row={}){return row.department || row.division_name || row.division || row.division_code || '';}
function getManagerTeam(row={}){return row.team || row.team_name || row.team_code || '';}
function isAlreadyWastewaterOperator(employeeNo){return wastewaterAdminState.operators.some(op=>String(op.employee_no||'')===String(employeeNo) && op.is_active!==false) || wastewaterAdminState.admins.some(ad=>String(ad.employee_no||'')===String(employeeNo));}
function isAlreadyWastewaterApprover(employeeNo){return wastewaterAdminState.approvers.some(ap=>String(ap.employee_no||'')===String(employeeNo) && ap.is_active!==false);}
function renderAdminSearchRows(){
  const rows=wastewaterAdminState.searchResults||[];
  if(!rows.length) return `<tr><td colspan="7">검색된 사원이 없습니다.</td></tr>`;
  return rows.map((row,index)=>{
    const employeeNo=getManagerEmployeeNo(row);
    const isOp=isAlreadyWastewaterOperator(employeeNo);
    const isAp=isAlreadyWastewaterApprover(employeeNo);
    return `<tr>
      <td>${escapeHtml(employeeNo)}</td>
      <td>${escapeHtml(row.name||'')}</td>
      <td>${escapeHtml(getManagerDepartment(row))}</td>
      <td>${escapeHtml(getManagerTeam(row))}</td>
      <td>${escapeHtml(row.position||'')}</td>
      <td>${escapeHtml(getManagerEmail(row))}</td>
      <td style="min-width:210px;">
        ${isOp?`<span class="inline-badge wastewater">운영자</span>`:`<button class="small-btn" onclick="addWastewaterOperatorFromSearch(${index})">운영자 지정</button>`}
        ${isAp?`<span class="inline-badge partial">결재자</span>`:`<button class="small-btn" onclick="addWastewaterApproverFromSearch(${index})">결재자 지정</button>`}
      </td>
    </tr>`;
  }).join('');
}
...
function renderManagerRows(rows=[],type='operator'){
  ...
  return rows.map(row=>{
    ...
    const action=isAdmin?`<span class="inline-badge wastewater">자동 포함</span>`:`<button class="small-btn danger" onclick="${type==='approver'?'removeWastewaterApprover':'removeWastewaterOperator'}('${escapeHtml(employeeNo)}')">해제</button>`;
    return `<tr>
      <td>${escapeHtml(employeeNo)}</td>
      <td>${escapeHtml(row.name||'')}</td>
...
function normalizeActorText(v){
  return String(v||'').trim().toLowerCase();
}

function canEditRelatedDoc(doc={}){
  if(canManageRelatedDocs()) return true;
  const meEmail = normalizeActorText(user?.email);
  const meName = normalizeActorText(user?.name);
  const writers = [
    doc.updated_by,
    doc.created_by,
    doc.writer_email,
    doc.author_email
  ].map(normalizeActorText).filter(Boolean);
  return !!(writers.includes(meEmail) || (meName && writers.includes(meName)));
}
...
function getRelatedDocs(){
  return (referenceDocs||[])
    .filter(doc => String(doc.doc_key||'').startsWith('related_') || RELATED_DOC_CATEGORIES.includes(doc.category))
...
function renderRelatedDocEditor(){
  ...
  if(relatedDocEditId && !editingDoc) return `<div class="msg err">수정할 게시글을 찾을 수 없습니다.</div>`;
  if(editingDoc && !canEditRelatedDoc(editingDoc)) return `<div class="msg err">작성자 또는 운영자/관리자만 수정할 수 있습니다.</div>`;
  if(!editingDoc && !canManageRelatedDocs()) return '';

  const editTitle = editingDoc?.title || '';
  const editCategory = editingDoc?.category || relatedDocCategoryFilter;
  const editBody = editingDoc?.content || editingDoc?.body || '';
  const editFile = editingDoc?.file_name || editingDoc?.source || '';
  const modeTitle = editingDoc ? '게시글 수정' : `${escapeHtml(relatedDocCategoryFilter)} 글 작성`;

  return `
    <div class="related-doc-form">
      <div class="section-title">✍️ ${modeTitle}</div>
      <div class="field">
        <label>제목</label>
        <input id="relatedDocTitle" class="input" value="${escapeHtml(editTitle)}" placeholder="예: 2026년 폐수배출시설 설치 신고증명서">
      </div>
      <div class="field">
        <label>게시판 구분</label>
        <select id="relatedDocCategory" class="select">
          ${RELATED_DOC_CATEGORIES.map(cat=>`<option value="${escapeHtml(cat)}" ${cat===editCategory?'selected':''}>${escapeHtml(cat)}</option>`).join('')}
        </select>
      </div>
      <div class="field">
        <label>내용</label>
        ...
        <div id="relatedDocBody" class="rich-editor related-doc-editor" contenteditable="true" data-placeholder="서류 변경 이력, 보관 위치, 주요 내용 등을 입력하세요. 이미지도 삽입할 수 있습니다.">${editBody}</div>
      </div>
      <div class="field">
      ...
      <div class="toolbar-row" style="margin-bottom:0;">
        ...
        <div class="toolbar-right">
          <button type="button" class="btn soft" onclick="event.preventDefault(); closeRelatedDocWriter(); return false;">목록으로</button>
          <button type="button" id="relatedDocSaveBtn" class="btn primary" onclick="event.preventDefault(); saveRelatedDoc(); return false;">${editingDoc?'수정 저장':'게시글 저장'}</button>
        </div>
      </div>
    </div>
  `;
...
function renderRelatedDocDetail(){
  const doc = getRelatedDocs().find(item => String(item.id) === String(relatedDocViewId));
  if(!doc){
    relatedDocViewId=null;
    return `<div class="msg muted">선택한 게시글을 찾을 수 없습니다.</div>`;
  }

  const title=doc.title || '제목 없음';
  const category=doc.category || RELATED_DOC_CATEGORIES[0];
  const body=doc.content || doc.body || '<p>본문 없음</p>';
  const fileName=doc.file_name || doc.source || '';
  const writer=doc.updated_by || doc.created_by || '-';
  const date=doc.updated_at || doc.created_at
    ? new Date(doc.updated_at || doc.created_at).toLocaleString('ko-KR')
    : '-';

  return `
    <div class="related-doc-detail">
      <div class="toolbar-row">
        <div>
          <div class="related-doc-detail-category">${escapeHtml(category)}</div>
          <div class="related-doc-detail-title">${escapeHtml(title)}</div>
          <div class="related-doc-meta">작성일: ${escapeHtml(date)} · 작성자: ${escapeHtml(writer)}</div>
        </div>
        <div class="toolbar-right">
          <button type="button" class="btn soft" onclick="closeRelatedDocDetail()">목록으로</button>
          ${canEditRelatedDoc(doc)?`<button type="button" class="btn" onclick="editRelatedDoc('${doc.id}')">수정</button>`:''}
          ${canManageRelatedDocs()?`<button type="button" class="btn danger" onclick="deleteRelatedDoc('${doc.id}')">삭제</button>`:''}
        </div>
      </div>
      ${fileName?`<div class="related-doc-file">📎 ${escapeHtml(fileName)}</div>`:''}
...
function openRelatedDocDetail(id){
  relatedDocViewId=id;
  relatedDocWriteMode=false;
  renderApp();
}

function closeRelatedDocDetail(){
  relatedDocViewId=null;
  renderApp();
}


function editRelatedDoc(id){
  const doc = getRelatedDocs().find(item => String(item.id) === String(id));
  if(!doc) return renderApp('수정할 게시글을 찾을 수 없습니다.','err');
  if(!canEditRelatedDoc(doc)) return renderApp('작성자 또는 운영자/관리자만 수정할 수 있습니다.','err');

  relatedDocEditId=id;
  relatedDocViewId=null;
  relatedDocWriteMode=true;
  relatedDocCategoryFilter=doc.category || relatedDocCategoryFilter;
  renderApp();
}
...
function renderRelatedDocsPanel(){
  ...
  const formHtml = relatedDocWriteMode ? renderRelatedDocEditor() : '';
  const writeButton = canManageRelatedDocs() && !relatedDocWriteMode && !relatedDocViewId
    ? `<button class="btn primary" onclick="openRelatedDocWriter()">글쓰기</button>`
    : '';
  ...
  return `
    <div class="card ref-card">
      <div class="related-doc-toolbar">
        ...
        <div class="toolbar-right">${writeButton}</div>
      </div>
      ...
      <div class="related-doc-layout">
        ...
        <div class="related-doc-content">
          ${relatedDocViewId ? renderRelatedDocDetail() : (relatedDocWriteMode ? formHtml : renderRelatedDocTable(rows))}
        </div>
      </div>
    </div>
  `;
...
function openRelatedDocWriter(){
  if(!canManageRelatedDocs()) return renderApp('운영자 또는 관리자만 글을 작성할 수 있습니다.','err');
...
function closeRelatedDocWriter(){
  relatedDocWriteMode=false;
  relatedDocEditId=null;
  renderApp();
}
...
async function saveRelatedDoc(){
  const msgEl=document.getElementById('relatedDocSaveMsg');
  const btn=document.getElementById('relatedDocSaveBtn');
  const showSaveMsg=(msg,cls='muted')=>{
    if(msgEl){
  ...
  if(editingDoc && !canEditRelatedDoc(editingDoc)){
    showSaveMsg('작성자 또는 운영자/관리자만 수정할 수 있습니다.','err');
    return;
  }
  if(!editingDoc && !canManageRelatedDocs()){
    showSaveMsg('운영자 또는 관리자만 등록 가능합니다.','err');
    return renderApp('운영자 또는 관리자만 등록 가능합니다.','err');
  }
  ...
  const bodyEl=document.getElementById('relatedDocBody');
  ...
  if(!editingDoc){
    payload.created_by=user?.name || user?.email || '';
  ...
  try{
    if(editingDoc){
      if(api && typeof api.saveRelatedDocument==='function'){
        result = await api.saveRelatedDocument(editingDoc.id,payload);
      }else if(api && typeof api.saveReferenceDoc==='function'){
        result = await api.saveReferenceDoc(editingDoc.id,payload);
      }else if(window.sb){
        result = await window.sb.from('reference_docs').update(payload).eq('id', editingDoc.id).select('*').maybeSingle();
      }else{
        throw new Error('Supabase 연결 객체를 찾을 수 없습니다.');
      }
    }else{
      if(api && typeof api.saveRelatedDocument==='function'){
        result = await api.saveRelatedDocument(null,payload);
      }else if(api && typeof api.saveReferenceDoc==='function'){
        result = await api.saveReferenceDoc(null,payload);
      }else if(window.sb){
        result = await window.sb.from('reference_docs').insert([payload]).select('*').maybeSingle();
      }else{
        throw new Error('Supabase 연결 객체를 찾을 수 없습니다.');
      }
    }
    ...
    showSaveMsg('관련서류 저장 실패: '+(err?.message || err),'err');
    return;
  }

  if(result?.error){
    if(btn) btn.disabled=false;
    showSaveMsg('관련서류 저장 실패: '+result.error.message,'err');
    return;
  }

  const savedRow = result?.data || {...editingDoc, ...payload, id: editingDoc?.id};
  if(editingDoc){
    referenceDocs = (referenceDocs || []).map(row => String(row.id) === String(editingDoc.id) ? savedRow : row);
  }else{
    referenceDocs = [savedRow, ...(referenceDocs || [])];
  }
  ...
  try{
    await loadAll(editingDoc?'관련서류 게시글 수정 완료':'관련서류 게시글 저장 완료','ok');
  }catch(err){
    renderApp(editingDoc?'관련서류 게시글 수정 완료':'관련서류 게시글 저장 완료','ok');
  }
}
...
function renderAdminManagementPanel(){
  ...
  const approvers=wastewaterAdminState.approvers||[];
  return `<div class="admin-panel-grid">
    <div class="card admin-card">
      <div class="toolbar-row">
        <div>
          <div class="section-title">🔐 관리자 기능</div>
          <div class="msg muted">사원정보를 기준으로 폐수 운영자와 결재자를 지정합니다. 관리자는 포탈/사원정보의 admin 권한에서 자동 포함됩니다.</div>
        </div>
      </div>
      <div class="preview-grid">
        <div class="mini-card"><div class="mini-label">현재 관리자</div><div class="mini-value">${admins.length}명</div></div>
        <div class="mini-card"><div class="mini-label">현재 운영자</div><div class="mini-value">${operators.length}명</div></div>
        <div class="mini-card"><div class="mini-label">현재 결재자</div><div class="mini-value">${approvers.length}명</div></div>
        <div class="mini-card"><div class="mini-label">기준 정보</div><div class="mini-value">사원정보</div></div>
      </div>
    </div>

    <div class="card admin-card">
      <div class="section-title">🔎 권한 대상자 검색</div>
      <div class="msg muted">사번, 이름, 이메일로 검색한 뒤 운영자 또는 결재자로 지정합니다.</div>
      <div class="toolbar-row">
        <input id="wm-employee-keyword" class="input" placeholder="예: 100123 또는 홍길동" style="max-width:520px;margin:0" onkeydown="if(event.key==='Enter') searchWastewaterEmployees()">
        <div class="toolbar-right">
          <button class="btn primary" onclick="searchWastewaterEmployees()">사원 검색</button>
          <button class="btn soft" onclick="refreshWastewaterManagers()">권한 새로고침</button>
        </div>
      </div>
      <div class="table-wrap"><table><thead><tr><th>사번</th><th>이름</th><th>부서</th><th>팀</th><th>직급</th><th>이메일</th><th>작업</th></tr></thead><tbody>${renderAdminSearchRows()}</tbody></table></div>
    </div>
    ...
    <div class="card log-card">
      ...
      <div class="msg muted">최근 200건</div>
      <div class="table-wrap">
        <table><thead><tr><th>시간</th><th>사용자</th><th>동작</th><th>대상</th><th>상세</th></tr></thead><tbody>
          ${logRows.length?logRows.map(r=>`<tr><td>${escapeHtml(r.created_at||'')}</td><td>${escapeHtml(r.actor_email||'')}</td><td>${escapeHtml(r.action||'')}</td><td>${escapeHtml(r.target_type||'')} / ${escapeHtml(r.target_id||'')}</td><td>${escapeHtml(r.details||'')}</td></tr>`).join(''):`<tr><td colspan="5">로그가 없습니다.</td></tr>`}
        </tbody></table>
      </div>
    </div>
  </div>`;
}
async function refreshWastewaterManagers(message='권한 목록을 새로고침했습니다.',cls='ok'){
  if(!canAdmin()) return renderApp('관리자만 사용할 수 있습니다.','err');
  if(api.loadWastewaterManagers){
    const res=await api.loadWastewaterManagers();
    wastewaterAdminState={...wastewaterAdminState,...(res.data||{})};
  }
  renderApp(message,cls);
}
async function searchWastewaterEmployees(){
  if(!canAdmin()) return renderApp('관리자만 사원을 검색할 수 있습니다.','err');
  const keyword=String(document.getElementById('wm-employee-keyword')?.value||'').trim();
  if(!keyword){ wastewaterAdminState.searchResults=[]; return renderApp('사번, 이름 또는 이메일을 입력하세요.','err'); }
  const {data,error}=await api.searchEmployees(keyword);
  if(error) return renderApp('사원 검색 실패: '+error.message,'err');
  wastewaterAdminState.searchResults=Array.isArray(data)?data:[];
  renderApp(wastewaterAdminState.searchResults.length?'사원 검색 완료':'검색된 사원이 없습니다.', wastewaterAdminState.searchResults.length?'ok':'muted');
  const input=document.getElementById('wm-employee-keyword'); if(input) input.value=keyword;
}
...
async function removeWastewaterOperator(employeeNo){
  ...
  if(!confirm('이 운영자 권한을 해제하시겠습니까?')) return;
  const {error}=await api.removeWastewaterOperator(employeeNo,getCurrentActorName());
  if(error) return renderApp('운영자 해제 실패: '+error.message,'err');
  await logActivity('wastewater_operator_remove','user',employeeNo,{});
  await loadAll('운영자 권한이 해제되었습니다.','ok');
}
...
async function removeWastewaterApprover(employeeNo){
  ...
  if(!confirm('이 결재자 권한을 해제하시겠습니까?')) return;
  const {error}=await api.removeWastewaterApprover(employeeNo,getCurrentActorName());
  if(error) return renderApp('결재자 해제 실패: '+error.message,'err');
  await logActivity('wastewater_approver_remove','user',employeeNo,{});
  await loadAll('결재자 권한이 해제되었습니다.','ok');
}
...
<div class="field"><label>날짜</label><input id="daily-date" class="input" type="date" oninput="updateDailyPreview()"></div>
...
<div class="checkbox-row"><input id="daily-is-holiday" type="checkbox" onchange="toggleHolidayBox(); updateDailyPreview();"><label for="daily-is-holiday">휴일 입력</label></div>
<div id="holiday-box" style="display:none;">
<div class="field"><label>휴일 사유</label><input id="daily-holiday-reason" class="input" placeholder="예: 회사 단체휴무, 임시휴무, 샌드위치 휴무"></div>
</div>
...
<div id="pickup-msg" class="msg muted"></div></div>
<div class="card preview-card">
<div class="toolbar-row"><div><div class="section-title">📋 수거내역</div><div class="msg muted">폐수는 운영일지 반영, 폐기물은 기록용 저장입니다.</div></div><div class="toolbar-right"><select class="select" style="width:140px;margin:0" onchange="changePickupYear(this.value)">${pickupYears.map(y=>`<option value="${y}" ${pickupYearFilter===y?'selected':''}>${y}년</option>`).join('')}</select><select class="select" style="width:140px;margin:0" onchange="changePickupTypeFilter(this.value)"><option value="전체" ${pickupTypeFilter==='전체'?'selected':''}>전체</option><option value="폐수" ${pickupTypeFilter==='폐수'?'selected':''}>폐수만</option><option value="폐기물" ${pickupTypeFilter==='폐기물'?'selected':''}>폐기물만</option></select></div></div>
<div class="table-wrap"><table><thead><tr><th>수거일</th><th>구분</th><th>세부내용</th><th>확인서번호</th><th>처리업소</th><th>비고</th><th>삭제</th></tr></thead><tbody>${renderPickupRows(filteredPickupRows)}</tbody></table></div>
</div></div></div>
...
<div class="ledger-top">
  <div>
    ...
    <div class="msg muted">검토자 결재 완료 시 해당월은 잠깁니다. 휴일도 별도 입력하여 기록으로 남길 수 있습니다.</div>
  </div>
  <div class="ledger-approval-box">
    <span class="inline-badge ${approvalStatus.cls}">${approvalStatus.text}</span>
    ${canWriterApprove()?`<button class="btn ${getApprovalRow(ledgerMonthFilter)?.writer_signed_at ? 'approval-on' : 'soft'}" onclick="toggleWriterApproval('${ledgerMonthFilter}')">${getApprovalRow(ledgerMonthFilter)?.writer_signed_at ? '작성자 결재완료' : '작성자 결재'}</button>`:''}
    ${canReviewerApprove()?`<button class="btn ${getApprovalRow(ledgerMonthFilter)?.reviewer_signed_at ? 'approval-on' : 'soft'}" onclick="toggleReviewerApproval('${ledgerMonthFilter}')">${getApprovalRow(ledgerMonthFilter)?.reviewer_signed_at ? '검토자 결재완료' : '검토자 결재'}</button>`:''}
...
<div class="table-wrap">
  <table>
    ...
    <tbody>${renderLedgerRows(filteredLedgerRows)}</tbody>
  </table>
</div>
</div>
...
function changeYearlyReportYear(v){yearlyReportFilter=v; renderApp();}
function changeRefTab(v){refTab=v; refSearch=''; renderApp();}
function changeRefSearch(v){refSearch=v; renderApp();}
function setPickupType(type){const hidden=document.getElementById('pickup-type'); if(hidden) hidden.value=type; togglePickupType();}
...
async function toggleWriterApproval(monthKey){
  const row = getApprovalRow(monthKey);
  if(row?.writer_signed_at){
    if(!confirm(`${monthKey} 작성자 결재를 취소하시겠습니까?`)) return;
    return unsignWriter(monthKey);
  }
  return signWriter(monthKey);
}

async function toggleReviewerApproval(monthKey){
  const row = getApprovalRow(monthKey);
  if(row?.reviewer_signed_at){
    if(!confirm(`${monthKey} 검토자 결재를 취소하시겠습니까?`)) return;
    return unsignReviewer(monthKey);
  }
  return signReviewer(monthKey);
}
async function signWriter(monthKey){
  if(!canWriterApprove()) return renderApp('작성자 결재 권한이 없습니다.','err');
  ...
  let result;
  if(row){
    result = await api.upsertWriterApproval(monthKey,payload,true);
  }else{
    result = await api.upsertWriterApproval(monthKey,payload,false);
  }

  if(result.error) return renderApp('작성자 결재 실패: ' + result.error.message, 'err');

  await logApprovalAction(monthKey,'writer_sign');
  approvalAlertDismissed=false;
...
.approval-alert-overlay{position:fixed;inset:0;background:rgba(15,23,42,.38);display:flex;align-items:center;justify-content:center;padding:24px;z-index:2000}
.approval-alert-modal{width:min(520px,100%);background:#fff;border:1px solid var(--line);border-radius:24px;box-shadow:0 24px 50px rgba(15,23,42,.18);overflow:hidden}
.approval-alert-head{padding:20px 24px;border-bottom:1px solid #e5e7eb;background:#f8fafc}
.approval-alert-eyebrow{font-size:13px;font-weight:700;color:var(--muted);margin-bottom:8px}
.approval-alert-title{margin:0;font-size:24px;font-weight:800;letter-spacing:-.02em;color:var(--navy)}
.approval-alert-body{padding:22px 24px 12px}
.approval-alert-desc{margin:0 0 16px;color:#475467;line-height:1.7}
.approval-alert-list{display:flex;flex-wrap:wrap;gap:10px;margin:0 0 6px;padding:0;list-style:none}
.approval-alert-item{display:flex;align-items:center;gap:8px;padding:10px 12px;border:1px solid #dbe4ee;border-radius:999px;background:#fff}
.approval-alert-month{font-weight:800;color:var(--navy)}
.approval-alert-foot{display:flex;justify-content:flex-end;gap:10px;padding:0 24px 22px}
.portal-header,.stats,.tabs-wrap,.tab-panel{width:100%}

@media (max-width:720px){.approval-alert-modal{border-radius:20px}.approval-alert-title{font-size:22px}.approval-alert-foot{flex-direction:column}.approval-alert-foot .btn{width:100%;justify-content:center}}
...
function buildYearlyReportHtml(year){
  ...
  const includedMonthCount=data.monthlyRows.length||0;
  const avgWater=includedMonthCount?round2(data.totalWaterUse/includedMonthCount):0;
  const avgEntrusted=includedMonthCount?round2(data.totalEntrusted/includedMonthCount):0;
  const avgOver=includedMonthCount?round2(data.totalOverCount/includedMonthCount):0;
  const totalWasteCount=data.wasteTotals.alkali+data.wasteTotals.acid+data.wasteTotals.organicSolid+data.wasteTotals.organicBottle;
  const avgWaste=includedMonthCount?round2(totalWasteCount/includedMonthCount):0;
  const avgAlkali=includedMonthCount?round2(data.wasteTotals.alkali/includedMonthCount):0;
  const avgAcid=includedMonthCount?round2(data.wasteTotals.acid/includedMonthCount):0;
  const avgSolid=includedMonthCount?round2(data.wasteTotals.organicSolid/includedMonthCount):0;
  const avgBottle=includedMonthCount?round2(data.wasteTotals.organicBottle/includedMonthCount):0;

  const chartRows=data.allMonthlyRows||data.monthlyRows;
  const waterChart=makeBarChartSvg({title:'월별 용수 사용량',labels:chartRows.map(r=>r.label),values:chartRows.map(r=>round2(r.waterUse)),unit:'m³',color:'#2563eb'});
  const entrustedChart=makeBarChartSvg({title:'월별 폐수 위탁량',labels:chartRows.map(r=>r.label),values:chartRows.map(r=>round2(r.entrusted)),unit:'m³',color:'#0f9d58'});
  const overChart=makeBarChartSvg({title:'월별 OVER 발생 횟수',labels:chartRows.map(r=>r.label),values:chartRows.map(r=>r.overCount),unit:'일',color:'#f04438'});
  const wasteTotalChart=makeBarChartSvg({title:'폐기물 종류별 총합',labels:['폐알칼리','폐산','고상','폐시약병'],values:[data.wasteTotals.alkali,data.wasteTotals.acid,data.wasteTotals.organicSolid,data.wasteTotals.organicBottle],unit:'개수',color:'#f79009'});
  const wasteMonthlyStackedChart=makeStackedBarChartSvg({title:'월별 폐기물 추이',labels:chartRows.map(r=>r.label),series:[{name:'폐알칼리',values:chartRows.map(r=>r.wasteAlkali)},{name:'폐산',values:chartRows.map(r=>r.wasteAcid)},{name:'고상',values:chartRows.map(r=>r.wasteOrganicSolid)},{name:'폐시약병',values:chartRows.map(r=>r.wasteOrganicBottle)}],colors:['#f79009','#f04438','#12b76a','#6172f3'],unit:'개수'});
  const monthlyRowsHtml=data.monthlyRows.map(r=>`<tr><td>${escapeHtml(r.label)}</td><td class="num">${formatNum(r.waterUse,2)}</td><td class="num">${formatNum(r.entrusted,2)}</td><td class="num">${r.overCount}</td><td class="num">${r.wasteAlkali}</td><td class="num">${r.wasteAcid}</td><td class="num">${r.wasteOrganicSolid}</td><td class="num">${r.wasteOrganicBottle}</td></tr>`).join('');
  const averageRowHtml=includedMonthCount?`<tr style="font-weight:800;background:#f8fafc;"><td>월평균</td><td class="num">${formatNum(avgWater,2)}</td><td class="num">${formatNum(avgEntrusted,2)}</td><td class="num">${formatNum(avgOver,2)}</td><td class="num">${formatNum(avgAlkali,2)}</td><td class="num">${formatNum(avgAcid,2)}</td><td class="num">${formatNum(avgSolid,2)}</td><td class="num">${formatNum(avgBottle,2)}</td></tr>`:'';
  const approvalBox=`<div class="section"><div class="section-title">연간 결재 상태</div><div class="box small"><div><b>결재 완료 월:</b> ${approval.completedCount} / ${approval.totalMonths}</div>${approval.isComplete?`<div style="margin-top:8px;color:#027a48;font-weight:800;">✔ 연간 데이터 확정</div>`:`<div style="margin-top:8px;color:#b42318;"><b>미결재 월:</b> ${escapeHtml(approval.pendingMonths.join(', '))}</div>`}<div style="margin-top:8px;color:#667085;">※ 당월 데이터는 제외되고, 익월부터 연간 취합에 반영됩니다.</div></div></div>`;

  return `<html><head><meta charset="UTF-8"><title>연간 운영 보고서</title>${getReportBaseStyle()}</head><body><div class="page"><div class="header"><div class="title">폐수배출시설 연간 운영 보고서</div><div class="sub">${escapeHtml(year)}년 기준 / 취합 반영 월수 ${includedMonthCount}개월</div></div><div class="kpi-grid"><div class="kpi kpi-accent water"><div class="kpi-icon">💧</div><div class="kpi-label">연간 총 용수 사용량</div><div class="kpi-value">${formatNum(data.totalWaterUse,2)} m³</div><div class="kpi-sub">월평균 ${formatNum(avgWater,2)} m³</div></div><div class="kpi kpi-accent wastewater"><div class="kpi-icon">♻️</div><div class="kpi-label">연간 총 폐수 위탁량</div><div class="kpi-value">${formatNum(data.totalEntrusted,2)} m³</div><div class="kpi-sub">월평균 ${formatNum(avgEntrusted,2)} m³</div></div><div class="kpi kpi-accent over"><div class="kpi-icon">⚠️</div><div class="kpi-label">연간 OVER 발생일수</div><div class="kpi-value">${data.totalOverCount}일</div><div class="kpi-sub">월평균 ${formatNum(avgOver,2)} 일</div></div><div class="kpi kpi-accent waste"><div class="kpi-icon">🧪</div><div class="kpi-label">연간 폐기물 총 수량</div><div class="kpi-value">${totalWasteCount}개</div><div class="kpi-sub">월평균 ${formatNum(avgWaste,2)} 개</div></div></div>${approvalBox}<div class="three-col"><div class="box small"><div style="font-weight:800;margin-bottom:6px;">용수 사용량 최대 월</div><div>${escapeHtml(data.topWaterMonth?.label||'-')} / ${formatNum(data.topWaterMonth?.waterUse||0,2)}m³</div></div><div class="box small"><div style="font-weight:800;margin-bottom:6px;">위탁량 최대 월</div><div>${escapeHtml(data.topEntrustedMonth?.label||'-')} / ${formatNum(data.topEntrustedMonth?.entrusted||0,2)}m³</div></div><div class="box small"><div style="font-weight:800;margin-bottom:6px;">OVER 집중 월</div><div>${escapeHtml(data.topOverMonth?.label||'-')} / ${data.topOverMonth?.overCount||0}일</div></div></div><div class="section">${waterChart}</div><div class="section">${entrustedChart}</div></div><div class="page"><div class="header"><div class="title">연간 분석 및 폐기물 추이</div><div class="sub">${escapeHtml(year)}년 기준</div></div><div class="section">${overChart}</div><div class="section">${wasteTotalChart}</div><div class="section">${wasteMonthlyStackedChart}</div></div><div class="page"><div class="header"><div class="title">월별 요약표</div><div class="sub">${escapeHtml(year)}년 기준</div></div><div class="section"><table><thead><tr><th>월</th><th>용수 사용량</th><th>폐수 위탁량</th><th>OVER 일수</th><th>폐알칼리</th><th>폐산</th><th>고상</th><th>폐시약병</th></tr></thead><tbody>${monthlyRowsHtml||`<tr><td colspan="8">취합된 데이터가 없습니다.</td></tr>`}${averageRowHtml}</tbody></table></div></div></body></html>`;
}
function openReportPrintWindow(html){const win=window.open('','_blank'); if(!win) return renderApp('팝업이 차단되어 보고서를 열 수 없습니다.','err'); win.document.open(); win.document.write(html); win.document.close(); setTimeout(()=>{win.focus(); win.print();},500);}
function downloadMonthlyReportPdf(){if(!canDownload()) return renderApp('월보고서 출력 권한이 없습니다.','err'); const month=document.getElementById('ledger-month')?.value||ledgerMonthFilter; if(!month) return renderApp('월보고서 기준 월이 없습니다.','err'); openReportPrintWindow(buildMonthlyReportHtml(month));}
function downloadYearlyReportPdf(){if(!canDownload()) return renderApp('연보고서 출력 권한이 없습니다.','err'); const year=yearlyReportFilter||getDefaultYearlyReportYear(); if(!year) return renderApp('연보고서 기준 연도가 없습니다.','err'); openReportPrintWindow(buildYearlyReportHtml(year));}
function downloadMonthlyExcel(){if(!canDownload()) return renderApp('엑셀 다운로드 권한이 없습니다.','err'); const month=document.getElementById('ledger-month')?.value||ledgerMonthFilter||monthKeyFromDate(new Date().toISOString().slice(0,10)); const stats=getDerivedStats(); const ledgerRows=[...stats.enrichedDaily].filter(r=>monthKeyFromDate(r.date)===month).sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.created_at||'').localeCompare(String(a.created_at||''))); const pickupFiltered=pickupRows.filter(r=>monthKeyFromDate(r.pickup_date)===month).sort((a,b)=>String(b.pickup_date).localeCompare(String(a.pickup_date))||String(b.created_at||'').localeCompare(String(a.created_at||''))); const logsFiltered=logRows.filter(r=>monthKeyFromDate(r.created_at)===month); const sheet1=[['날짜','용수전일','용수금일','당일사용량','저장고높이','외부보관','계산용총높이','금일지침','총발생량','수거정보','기타'],...ledgerRows.map(r=>[r.date,formatNum(r.water_prev,2),formatNum(r.usage,2),formatNum(r.water_used,2),formatNum(r.height,1),r.has_external?`${formatNum(r.external_ton||0,1)}T / ${formatNum(r.external_cm,1)}cm`:'-',formatNum(r.total_cm,1),r.guideline_text,r.generated_text,r.pickup?`위탁 ${formatNum(r.pickup.entrusted_amount,2)} / 후높이 ${String(r.pickup.after_pickup_cm??'-')}`:'-',r.note||''])]; const sheet2=[['수거일','구분','세부내용','확인서번호','처리업소','비고'],...pickupFiltered.map(r=>[r.pickup_date,r.pickup_type||'폐수',getPickupDetailText(r),r.certificate_no||'',r.contractor||'',r.note||''])]; const sheet3=[['시간','사용자','동작','대상','상세'],...logsFiltered.map(r=>[r.created_at||'',r.actor_email||'',r.action||'',`${r.target_type||''}/${r.target_id||''}`,r.details||''])]; const workbookHtml=`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"></head><body>${tableToHtml('운영일지',sheet1)}<br/><br/>${tableToHtml('수거등록',sheet2)}<br/><br/>${tableToHtml('로그',sheet3)}</body></html>`; downloadFile(`폐수운영일지_${month}.xls`,workbookHtml,'application/vnd.ms-excel;charset=utf-8;');}
...
window.saveRelatedDoc = saveRelatedDoc;
window.closeRelatedDocWriter = closeRelatedDocWriter;
window.openRelatedDocWriter = openRelatedDocWriter;
window.changeRelatedDocCategory = changeRelatedDocCategory;
