  ...
  return `
    <div class="related-doc-form">
      ...
      <div class="toolbar-row" style="margin-bottom:0;">
        ...
        <div class="toolbar-right">
          <button class="btn soft" onclick="closeRelatedDocWriter()">목록으로</button>
...
async function saveRelatedDoc(){
  ...
  try{
    if(api && typeof api.saveRelatedDocument==='function'){
      ...
      result = await sb.from('reference_docs').insert([payload]).select('*').maybeSingle();
    }
  }catch(err){
    return renderApp('관련서류 저장 실패: '+(err?.message || err),'err');
  }

  if(result?.error){
    return renderApp('관련서류 저장 실패: '+result.error.message,'err');
  }
